...
class Standardize(Subfunction_Base):
    """A Standardization method which utilizes custom normalization functions
      functionality in order to normalize intensity value ranges to be
    ...
    def normalize(self, image):
        ...
        # Perform MinMax normalization between [0,1]
            ...
            # Scaling
            image_norm = (image - min_value + self.e) / (max_value - min_value + self.e)
            ...
            if np.min(image) < 0 or np.max(image) > 255:
                raise ValueError(
                    "Subfunction Standardize: Image values are not in range [0,255]!",
                    "Provided min/max values for image are:",
                    np.min(image),
                    np.max(image),
            ...
            if self.mode == "tf":
                ...
                image_norm = image.copy()
                for c in range(0, image.shape[-1]):
                    image_norm[..., c] = (image[..., c] - means[c]) / stds[c]
