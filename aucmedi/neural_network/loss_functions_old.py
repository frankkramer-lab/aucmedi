...
class BinaryFocalLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        if y_true.shape[1] != 1:
            raise ValueError(
                f"Expected y_true shape (batch_size, 1) but got {y_true.shape}. For multi-label classification, use MultilabelFocalLoss instead."
            )
        # Perform sigmoid on y_pred to get probabilities
...
class CategoricalFocalLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        # Perform softmax on y_pred to get probabilities
